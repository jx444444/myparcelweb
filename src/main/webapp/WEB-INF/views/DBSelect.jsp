...
<table border='1' cellpadding='8'>
	...
	<c:forEach var="item" items="${service.selectMember()}" varStatus="status">
		...
		<tr>
			<td>${item.index}<input type="button" value="삭제" onclick="location.href='/userdelete?uindex=${item.index}'"></td>
		   	<td colspan='4'>
		   		BASKET
...
<table border='1' cellpadding='10'>
	...
	<c:forEach var="item" items="${service.selectProduct()}" varStatus="status">
		<tr>
		   <td>${item.index}<input type="button" value="삭제" onclick="location.href='/productdelete?pindex=${item.index}'"></td>
		   <td>${decoder.Base64StringEncode(item.name)}</td>
...
<table border='1' cellpadding='8'>
		...
		<tr>
			<td>가격(pay)</td>
			<td><input type="number" value='0' name="pay"></td>
		</tr>
		<tr>
			<td>비고(explanatory)</td>
			<td><input type="text" name="explanatory"></td>
		</tr>
		<tr>
			<td>등록자(by)</td>
		...
		<tr>
			...
			<td>
			<select name="kind">
				<c:forEach var="productkind" items="${strings_productkind}" varStatus="status" begin="1">
					<option value="${status.index-1}">${productkind}</option>
				</c:forEach>
			</select>
			</td>
		</tr>
